...
export async function createRecharge({
	userId,
	amount,
}: typeof CreateRechargeInput.$inferInput) {
	await getUser(userId);
	const recharge = await prisma.recharge.create({
		data: { amount, userId, status: RECHARGE_STATUS.PENDING },
	});
	return recharge;
}
...
export async function updateRecharge(
	id: string,
	{ amount, status }: typeof UpdateRechargeInput.$inferInput,
) {
	...
	if (status === RECHARGE_STATUS.SUCCEEDED) {
		return await prisma.$transaction(async t => {
			await t.user.update({
				where: { id: oldRecharge.userId },
				data: { balance: { increment: oldRecharge.amount } },
			});
			return await t.recharge.update({
				where: { id },
				data: { amount, status },
			});
		});
