...
import { CreateUserInput, UsersFilter, UsersOrderBy } from "../entities";
...
export async function createUser({ name }: typeof CreateUserInput.$inferInput) {
  const user = await prisma.user.create({ data: {name, balance: new Prisma.Decimal(0)}})
  return user
}

export async function updateUser(id: string, { name }: typeof CreateUserInput.$inferInput) {
  await getUser(id)
