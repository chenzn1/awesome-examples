...
import { useUpdateRechargeMutation, GetRechargesQuery } from "@/client/graphql/__generated__/recharge.generated"
...
export function UpdateRechargeModal({recharge}: UpdateRechargeModalProps) {
  ...
  const [status, setStatus] = useState<Recharge_Status>(Recharge_Status.Pending)
  const { toast } = useToast()

  const {mutate, isPending} = useUpdateRechargeMutation({onSuccess: () => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Select onValueChange={status => setStatus(status as Recharge_Status)} value={status}>
              <SelectTrigger className="w-[180px]">
                <SelectValue  />
              </SelectTrigger>
              <SelectContent>
                  <SelectItem value={Recharge_Status.Pending}>Pending</SelectItem>
                  <SelectItem value={Recharge_Status.Succeeded}>Succeeded</SelectItem>
        ...
        <DialogFooter>
          ...
          <Button type="submit" disabled={isPending} onClick={() => mutate({id: recharge.id, data: {amount, status}})}>Update</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
